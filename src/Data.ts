export class DesignNode {
  ...
  static fromSceneNode(node: any) {
    const protoNode = new DesignNode(node.id, node.name, node.type);
    Object.keys(protoNode)
      .filter((key) => key !== "children")
      .forEach((key) => {
        protoNode[key] = node[key];
      });
    if (node.children) {
      protoNode.children = node.children.map((child) =>
        DesignNode.fromSceneNode(child)
      );
    }
    return protoNode;
  }
